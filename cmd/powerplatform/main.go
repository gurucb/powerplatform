...
func main() {
	run := func() int {
		...
		ctx, log := m.StartRootSpan(ctx, "powerplatform")
		defer func() {
...
func buildRootCommand(m *powerplatform.Mixin, in io.Reader) *cobra.Command {
	cmd := &cobra.Command{
		Use:  "powerplatform",
		Long: "A Power Platform mixin to use for deploying to Power Platform",
